...
import tw.momocraft.regionplus.utils.BungeeCord;
import tw.momocraft.regionplus.utils.Utils;

public class ServerHandler {
	public static void sendConsoleMessage(String message) {
		String prefix = "&7[&dRegionPlus&7] ";
		message = prefix + message;
		message = ChatColor.translateAlternateColorCodes('&', message).toString();
		RegionPlus.getInstance().getServer().getConsoleSender().sendMessage(message);
	}

	public static void sendDebugMessage(String message) {
		if (ConfigHandler.getDebugging()) {
			String prefix = "&7[&dRegionPlus_Debug&7] ";
			message = prefix + message;
	...
	public static void sendErrorMessage(String message) {
		String prefix = "&7[&cRegionPlus_ERROR&7]&c ";
		message = prefix + message;
		message = ChatColor.translateAlternateColorCodes('&', message);
		RegionPlus.getInstance().getServer().getConsoleSender().sendMessage(message);
	}
