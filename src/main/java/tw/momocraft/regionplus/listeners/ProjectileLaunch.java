                ...
                if (e.getEntity().getShooter() instanceof Player) {
                    ...
                    if (visitorProp != null) {
                        ...
                        for (String groupName : visitorProp.keySet()) {
                            ...
                            if (!visitorMap.isItemsProjectile()) {
                                return;
                            }
                            // Location
                            loc = player.getLocation();
                            if (RegionUtils.bypassBorder(player, loc, visitorMap.getLocMaps())) {
                                ServerHandler.sendFeatureMessage("Visitor", entityType, "Use-Items: Projectile-Location", "return", groupName,
                            ...
                            if (ConfigHandler.getDepends().ItemJoinEnabled()) {
                                if (visitorMap.isItemJoin()) {
                                    ...
                                    if (itemJoinAPI.isCustom(player.getInventory().getItemInMainHand())) {
                                        ServerHandler.sendFeatureMessage("Visitor", entityType, "Use-Items: Projectile-ItemJoin", "bypass", groupName,
                                                new Throwable().getStackTrace()[0]);
                                        return;
                                    }
                                }
                            ...
                            ServerHandler.sendFeatureMessage("Visitor", entityType, "Use-Items: Projectile-Final", "cancel", groupName,
                                    new Throwable().getStackTrace()[0]);
                            e.setCancelled(true);
                            return;
                        }
